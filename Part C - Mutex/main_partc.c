...
process consumer(sid32, sid32), producer(sid32, sid32);
...
process producer(sid32 consumed, sid32 produced)
{
	int32 i=0;
	while(1)
	{
...
process consumer(sid32 consumed, sid32 produced)
{	
	int32 n;
	while(1)
	{
...
{	
	...
	producer_id = create(producer, 4096, 50, "producer", 2, consumed, produced);
	consumer_id = create(consumer, 4096, 50, "consumer", 2, consumed, produced);
...
TIME ELAPSED (100): 336
TIME ELAPSED (200): 710
TIME ELAPSED (300): 1061
TIME ELAPSED (400): 1417
TIME ELAPSED (500): 1851
